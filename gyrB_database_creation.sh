...
for (( i=0; i<18997; i++)); 
	do 
		...
		awk "/^>/ {n++} n>1 {exit} {print}" 16S.txt >temp.txt; awk '/^>/{print ">'$(head -n 1 name.txt)'" ; next}{print}' < gyrB.txt > $(head -n 1 name.txt).gyr.fa;
		awk '/^>/{print ">'$(head -n 1 name.txt)'" ; next}{print}' < temp.txt > $(head -n 1 name.txt).16S.frn; mv $(head -n 1 name.txt).gyr.fa ../../../Gyr; mv $(head -n 1 name.txt).16S.frn ../../../16S; 
done
